# Comp 781 Final Project: Robot Crowd Navigation
This project builds upon the codebase from [Towards Generalizable Safety in Crowd Navigation via Conformal Uncertainty Handling](https://arxiv.org/abs/2508.05634v1)_. We extended the original simulation testing across different robot sizes and crowd densities, then incorporated real-world pedestrian trajectory data to evaluate the system in more realistic crowd environments. For the first task, we used custom_tests.py for testing results and custom_visualize.py for visualizations. For the second task, we added the directory "extract_data" to the codebase. This contains the videos that we extracted data from and our annotation scripts. 
